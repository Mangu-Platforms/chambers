...
function Bar({ w = "100%", h = "h-[3px]", tone = "muted" }: { w?: string; h?: string; tone?: "ink" | "muted" | "faint" | "accent" }) {
  const bg = tone === "ink" ? "bg-ink" : tone === "accent" ? "bg-vermilion/70" : tone === "faint" ? "bg-ink/10" : "bg-ink/20";
...
export function PaperThumb({ template, className }: { template: TemplateId; className?: string }) {
  return (
    <div data-template={template} className={cn("paper-thumb", template === "editorial" && "bg-cream", className)} aria-hidden>
      {template === "compact" ? (
