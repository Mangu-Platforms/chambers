...
export function BrandMark({
  ...
  return (
    <Link to={to} className="flex items-center gap-2.5 font-semibold tracking-[-0.02em] text-ink">
      <span className={cn("grid place-items-center bg-vermilion text-paper", tile)} aria-hidden>
      ...
      {wordmark ? <span className={size === "sm" ? "text-[15px]" : "text-base"}>CHAMBERS</span> : null}
    </Link>
  );
}
