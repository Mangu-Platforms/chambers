...
export const LetterSheet = memo(function LetterSheet({
  ...
  return (
    <article className="print-sheet letter-sheet w-full text-ink">
      <header>
        <h1 className="font-display m-0 text-[32px] font-semibold leading-[1.05] tracking-[-0.035em]">{name}</h1>
        ...
        <div className="mt-3 mb-5 flex flex-wrap gap-x-4 gap-y-1 text-xs text-soft">
          {[resume.identity.location, resume.identity.email, resume.identity.phone].filter(Boolean).map((c) => (
            <span key={c}>{c}</span>
          ))}
        </div>
        <p className="mb-7 text-xs text-soft">{today}</p>
      ...
      {paras.map((p) => (
        <p key={p.slice(0, 48)} className="mb-5 text-[15px] leading-[1.55] tracking-[-0.011em]">
          {p}
        </p>
      ))}
      <p className="mb-1 text-[15px]">{letter.closing || "Thank you for your time."}</p>
      <p className="mt-8 mb-0 text-[15px]">{(letter.signoff || "Sincerely").split("\n")[0].replace(/,$/, "")},</p>
