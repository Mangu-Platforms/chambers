...
import { ArrowRight, Clipboard, History } from "lucide-react";
...
export const Route = createFileRoute("/tailor")({
  ...
  head: () => ({ meta: [{ title: "Tailor · Chambers" }] }),
});
...
function TailorPage() {
  ...
  return (
    <div className="min-h-screen bg-fog">
      ...
      <div
        id="main"
        className="mx-auto grid max-w-page gap-8 px-5 py-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] md:px-8"
      >
        <TailorForm onTailored={() => setView("tailored")} />
        <div>
          {snapshot ? (
            <div className="no-print mb-4 flex w-fit rounded-pill border border-hair p-0.5">
              {(
                [
                  ["tailored", "Tailored"],
                  ["original", "Original"],
                ...
                <button
                  ...
                  className={cn(
                    ...
                    view === id ? "bg-ink text-paper" : "text-soft",
                  )}
                >
                  {label}
                </button>
...
function TailorForm({ onTailored }: { onTailored: () => void }) {
  ...
  const [busy, setBusy] = useState<"tailor" | "letter" | "refine" | null>(null);
  ...
  const runTailor = async () => {
    ...
    const ai = await raceAi(
      tailorResumeAi({ data: { resume, jobDescription } }),
      12000,
    );
    if (ai?.ok) {
      setResume(ai.resume);
  ...
  const runLetter = async () => {
    ...
    const local = writeLetterLocal(resume, jobDescription, targetCompany, targetRole, letterTone);
    setLetter(local);
    setBusy("letter");
    const ai = await raceAi(
      writeCoverLetterAi({
  ...
  return (
    <div className="no-print">
      ...
      <h1 className="font-serif mt-3 text-3xl font-medium tracking-[-0.03em]">
        One posting. One version.
      </h1>
      <p className="mt-2 text-body text-soft">
        Instant local reorder, then an optional refine. Missing requirements stay listed — never
        fabricated.
      </p>
      <div className="mt-6 flex flex-wrap gap-2">
        <Button
          ...
          onClick={() =>
            loadPosting(
              SAMPLE_JD_EDITORIAL,
              SAMPLE_TARGET_EDITORIAL.company,
              SAMPLE_TARGET_EDITORIAL.role,
            )
          }
        >
        ...
        <Button
          ...
          onClick={() =>
            loadPosting(
              SAMPLE_JD_ENGINEER,
              SAMPLE_TARGET_ENGINEER.company,
              SAMPLE_TARGET_ENGINEER.role,
            )
          }
        >
          ...
          onClick={async () => {
            try {
              ...
              if (text.trim().length < 40) {
                toast.error("Clipboard doesn’t look like a posting.");
                return;
              }
              setJobDescription(text);
              toast.success("Pasted posting");
            } catch {
              toast.error("Could not read the clipboard.");
            }
          }}
        >
          <Clipboard className="size-3.5" /> Paste posting
        </Button>
      </div>
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        <Field label="Company">
          <Input
            value={targetCompany}
            onChange={(e) => setTarget(e.target.value, targetRole)}
            placeholder="Acme Press Systems"
          />
        </Field>
        <Field label="Role">
          <Input
            value={targetRole}
            onChange={(e) => setTarget(targetCompany, e.target.value)}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Button disabled={busy !== null} onClick={() => void runTailor()}>
          {busy === "refine" ? "Refining…" : busy === "tailor" ? "Tailoring…" : "Tailor this resume"}
        </Button>
      ...
      {busy === "refine" ? (
        <p className="mt-3 text-caption text-soft">Sheet updated. Checking the assistant…</p>
      ) : null}
      <div className="mt-8 hairline rounded-lg bg-paper p-5">
        <LiveMatch />
      </div>
      {notes ? <NotesCard notes={notes} className="mt-4" /> : null}
      <KeywordsCard className="mt-4" />
      <Link
        to="/letter"
        className="mt-5 inline-flex min-h-11 items-center gap-1 text-sm font-semibold text-ember"
      >
